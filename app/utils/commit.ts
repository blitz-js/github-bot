...
type TreeMode = "100644" | "100755" | "040000" | "160000" | "120000" | undefined
type TreeType = "tree" | "blob" | "commit" | undefined

interface Change {
  message: string
  files?: Record<string, string | {contents: string; mode?: TreeMode; type?: TreeType}>
...
export default async function commit(args: {
  ...
  for (const change of changes) {
    ...
    const hasFiles = typeof change.files === "object" && Object.keys(change.files).length > 0
  ...
  for (const change of changes) {
    ...
    if (typeof change.files === "object" && Object.keys(change.files).length > 0) {
